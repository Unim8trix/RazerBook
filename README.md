# Arch Linux (Sway Wayland) on Razer Book 13 (RZ09-0357)

Theese are my personal notes setting up my Razer Book 13 with Arch Linux. Iam using full disk encryption and btrfs as filesystem.
...
I will try to have a pure wayland desktop. This and other tools i found usefull will be installed:

```bash
pacman -Syu sway alacritty firefox-developer-edition-i18n-de \
   cantarell-fonts ttf-font-awesome ttf-dejavu wofi waybar man-db \
   wl-clipboard swaybg mako swaylock acpi \
   wget git git-lfs bluez bluez-utils \
...
```bash
pacman -Sy xorg-xwayland xorg-xlsclients glfw-wayland
```
...
```bash
sudo pacman -Sy mc thunar gvfs gvfs-smb gvfs-nfs \
...
I use the distributed python version, it works well for my needs

```bash
sudo pacman -Sy ansible ansible-lint terraform
```

Add `PATH=$HOME/.local/bin/:$PATH` to the PATH variable in .zshrc
